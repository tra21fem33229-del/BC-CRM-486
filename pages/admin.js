...
function MiniTable({ rows, kind, cdsOnly }) {
        ...
        <tbody>
          {sorted.map((r, i) => (
            <tr key={r.key}>
              ...
              <td className="mono">{val(r, 'leadGiao')}</td>
              <td className="mono">{val(r, 'leadTuongTac') + val(r, 'oppTuongTac')}</td>
...
export default function Admin() {
  ...
  const [previewCds, setPreviewCds] = useState(false);
                ...
                {pendingData && (
                  <div style={{ marginTop: 18 }}>
                    <div className="actions-row" style={{ marginBottom: 4 }}>
                      <button
                        type="button"
                        className={`btn ${previewCds ? '' : 'secondary'}`}
                        style={{ padding: '6px 10px', fontSize: 11.5 }}
                        onClick={() => setPreviewCds((v) => !v)}
                      >
                        {previewCds ? '✓ Xem theo Sáng kiến CĐS' : 'Xem theo Sáng kiến CĐS'}
                      </button>
                    </div>
                    <div className="section-title">
                      <h2 style={{ fontSize: 14 }}>Xem trước — Top 5 Phòng{previewCds ? ' (CĐS)' : ''}</h2>
                    </div>
                    <MiniTable
                      rows={
                        previewCds
                          ? [...pendingData.phong]
                              .sort((a, b) => (b.diemCDS ?? -1) - (a.diemCDS ?? -1))
                              .slice(0, 5)
                          : pendingData.phong.slice(0, 5)
                      }
                      kind="phong"
                      cdsOnly={previewCds}
                    />
                    <div className="section-title">
                      <h2 style={{ fontSize: 14 }}>Xem trước — Top 5 Cán bộ{previewCds ? ' (CĐS)' : ''}</h2>
                    </div>
                    <MiniTable
                      rows={
                        previewCds
                          ? [...pendingData.rm].sort((a, b) => (b.diemCDS ?? -1) - (a.diemCDS ?? -1)).slice(0, 5)
                          : pendingData.rm.slice(0, 5)
                      }
                      kind="rm"
                      cdsOnly={previewCds}
                    />
