...
// Các giá trị cột "Nhóm nguồn" được tính là "Sáng kiến CĐS" khi bật nút lọc trên trang xếp hạng.
export const CDS_SOURCE_GROUPS = ['Kênh sáng kiến CĐS KHBL', 'Kênh sáng kiến CĐS KHDN'];

// Tăng số này mỗi khi thay đổi CẤU TRÚC của partial (ví dụ đổi khoá đối chiếu từ tên phòng
// sang mã phòng, thêm số liệu con lọc theo Sáng kiến CĐS, hoặc đổi cách lọc "Ngày tương tác" theo
// kỳ). Partial cũ không có đúng version này sẽ bị coi là "không dùng lại được", buộc admin phải
// tải lại file thay vì âm thầm dùng nhầm dữ liệu tính theo logic cũ.
export const PARTIAL_SCHEMA_VERSION = 5;
...
// "Ngày tương tác" trong file Excel (2 file "Tiếp cận tương tác") được XLSX đọc ra dạng chuỗi
// "DD-MM-YYYY" (do admin.js đọc file với option raw:false). Hàm này quy đổi về "YYYY-MM" để so
// khớp với kỳ tháng (monthKey, dạng input type="month" chuẩn HTML) đang được xử lý. Trả về ''
// nếu không đọc được ngày — dòng đó sẽ bị coi là NGOÀI kỳ (an toàn hơn là ngầm định trong kỳ).
export function monthKeyOfVNDate(raw) {
  const s = trim(raw);
...
export function buildLeadStatusPartial(rows) {
  ...
  //
  // Song song với số liệu đầy đủ, mỗi RM còn được cộng thêm số liệu "…CDS" — chỉ tính các dòng có
  // "Nhóm nguồn" thuộc CDS_SOURCE_GROUPS — để phục vụ nút bật/tắt "Sáng kiến CĐS" trên trang xếp
  // hạng, không cần xử lý lại file.
  const rmMap = {};
  rows.forEach((r) => {
    ...
    const isCDS = CDS_SOURCE_GROUPS.includes(trim(r['Nhóm nguồn']));
    ...
    if (isCDS) {
      rmMap[rm].leadGiaoCDS += giao;
      rmMap[rm].leadChuyenDoiCDS += chuyen;
    }
  });
...
export function buildOppStatusPartial(rows) {
  ...
  rows.forEach((r) => {
    ...
    if (isCDS) {
      rmMap[rm].oppTotalCDS += 1;
      rmMap[rm].oppThanhCongCDS += tc;
    }
  });
  return { rm: Object.values(rmMap) };
}

export function buildLeadIntPartial(rows, monthKey) {
  const rmMap = {}; // rm -> { phong, phongLabel, set(code), setCDS(code) }
  rows.forEach((r) => {
    const rm = trim(r['RM quản lý']);
    const code = trim(r['Lead code']);
    if (!rm || !code) return;
    // Chỉ tính Lead có "Ngày tương tác" rơi đúng vào kỳ tháng đang xử lý — file CRM xuất theo
    // trạng thái HIỆN TẠI nên vẫn liệt kê cả các Lead đã tương tác từ những kỳ trước nhưng chưa
    // đóng trạng thái; nếu không lọc sẽ đếm nhầm các tương tác cũ vào kỳ này.
    if (monthKey && monthKeyOfVNDate(r['Ngày tương tác']) !== monthKey) return;
    const maPhong = trim(r['Mã phòng']);
    const tenPhong = trim(r['Tên phòng']);
    const isCDS = CDS_SOURCE_GROUPS.includes(trim(r['Nhóm nguồn']));
    if (!rmMap[rm]) rmMap[rm] = { phong: maPhong, phongLabel: tenPhong, set: new Set(), setCDS: new Set() };
    if (maPhong) rmMap[rm].phong = maPhong;
    if (tenPhong) rmMap[rm].phongLabel = tenPhong;
    rmMap[rm].set.add(code);
    if (isCDS) rmMap[rm].setCDS.add(code);
  });
  return {
    rm: Object.entries(rmMap).map(([key, v]) => ({
      key,
      phong: v.phong,
      phongLabel: v.phongLabel,
      leadTuongTac: v.set.size,
      leadTuongTacCDS: v.setCDS.size,
    })),
  };
}

export function buildOppIntPartial(rows, monthKey) {
  const rmMap = {};
  rows.forEach((r) => {
    const rm = trim(r['RM quản lý']);
    const code = trim(r['Opp code']);
    if (!rm || !code) return;
    // Cùng lý do như buildLeadIntPartial: chỉ tính Opp có "Ngày tương tác" rơi đúng vào kỳ đang xử lý.
    if (monthKey && monthKeyOfVNDate(r['Ngày tương tác']) !== monthKey) return;
    const maPhong = trim(r['Mã phòng']);
    const tenPhong = trim(r['Tên phòng']);
    const isCDS = CDS_SOURCE_GROUPS.includes(trim(r['Nhóm nguồn']));
  ...
  return {
    rm: Object.entries(rmMap).map(([key, v]) => ({
      ...
      phong: v.phong,
      phongLabel: v.phongLabel,
      oppTuongTac: v.set.size,
      oppTuongTacCDS: v.setCDS.size,
    })),
  };
}
...
// monthKey (dạng "YYYY-MM", = tháng áp dụng đang chọn ở trang Quản trị) dùng để lọc "Ngày tương
// tác" — CHỈ áp dụng cho 2 file "Tiếp cận tương tác" (có cột này); nếu file không có cột "Ngày
// tương tác" thì bỏ qua lọc (giữ hành vi cũ) thay vì âm thầm loại hết dữ liệu.
export function buildPartialFromAOA(fileType, aoa, monthKey) {
  let result;
  if (fileType === 'roster') {
    const { rows, maPhongCol, rmCol, tenPhongCol } = rowsFromRosterAOA(aoa);
    result = buildRosterPartial(rows, maPhongCol, rmCol, tenPhongCol);
  } else {
    const { rows, headers } = rowsFromAOA(aoa);
    const hasNgayTuongTac = headers.includes('Ngày tương tác');
    if (fileType === 'lead_status') result = buildLeadStatusPartial(rows);
    else if (fileType === 'opp_status') result = buildOppStatusPartial(rows);
    else if (fileType === 'lead_int') result = buildLeadIntPartial(rows, hasNgayTuongTac ? monthKey : null);
    else if (fileType === 'opp_int') result = buildOppIntPartial(rows, hasNgayTuongTac ? monthKey : null);
...
export function validateAOA(fileType, aoa, monthKey) {
  if (fileType === 'roster') {
  ...
  if (fileType === 'lead_int' || fileType === 'opp_int') {
    if (!headers.includes('Ngày tương tác')) {
      warnings.push(
        'Không tìm thấy cột "Ngày tương tác" — KHÔNG lọc được theo kỳ, hệ thống sẽ tạm tính toàn bộ dòng trong file (kể cả tương tác từ kỳ trước).'
      );
    } else if (monthKey) {
      const total = rows.length;
      const outOfRange = rows.filter((r) => monthKeyOfVNDate(r['Ngày tương tác']) !== monthKey).length;
      if (outOfRange > 0) {
        warnings.push(
          `${outOfRange}/${total} dòng có "Ngày tương tác" ngoài kỳ ${monthKey} — các dòng này sẽ KHÔNG được tính vào số Lead/Opp có tương tác của kỳ này (bình thường vì file CRM xuất theo trạng thái hiện tại).`
        );
      }
    }
  }
...
export function scoreRM(rmRawRows) {
  return rmRawRows
    .map((r) => ({
      ...
      // diemCDS: cùng công thức Mục 6.2, chỉ tính trên số liệu con thuộc nhóm nguồn Sáng kiến CĐS.
      diemCDS: +(0.3 * ((r.leadTuongTacCDS || 0) + (r.oppTuongTacCDS || 0)) + 0.3 * (r.leadChuyenDoiCDS || 0) + 0.4 * (r.oppThanhCongCDS || 0)).toFixed(1),
    }))
...
export function scorePhong(phongRawRows, rmCountMap) {
  return phongRawRows
    .map((r) => {
      ...
      if (!soRM) return { ...r, soRM: 0, diem: null, diemCDS: null };
      const diem =
        0.3 * (((r.leadTuongTac || 0) + (r.oppTuongTac || 0)) / soRM) +
        0.3 * ((r.leadChuyenDoi || 0) / soRM) +
        0.4 * ((r.oppThanhCong || 0) / soRM);
      // diemCDS: cùng công thức Mục 6.1, chỉ tính trên số liệu con thuộc nhóm nguồn Sáng kiến CĐS,
      // vẫn chia cho ĐÚNG số RM biên chế của phòng (không đổi theo bộ lọc nguồn).
      const diemCDS =
        0.3 * (((r.leadTuongTacCDS || 0) + (r.oppTuongTacCDS || 0)) / soRM) +
        0.3 * ((r.leadChuyenDoiCDS || 0) / soRM) +
        0.4 * ((r.oppThanhCongCDS || 0) / soRM);
      return { ...r, soRM, diem: +diem.toFixed(2), diemCDS: +diemCDS.toFixed(2) };
    })
    .sort((a, b) => {
...
export function combinePartials(parts) {
  ...
  function ensureRm(k, phong, phongLabel) {
    if (!rmMapAll[k]) {
      rmMapAll[k] = {
        ...
        leadTuongTac: 0,
        oppTuongTac: 0,
        // Số liệu con — chỉ tính các dòng thuộc nhóm nguồn Sáng kiến CĐS (xem CDS_SOURCE_GROUPS).
        leadGiaoCDS: 0,
        leadChuyenDoiCDS: 0,
        oppTotalCDS: 0,
        oppThanhCongCDS: 0,
  ...
  (leadStatus?.rm || []).forEach((r) => {
    ...
    e.leadGiaoCDS += r.leadGiaoCDS || 0;
    e.leadChuyenDoiCDS += r.leadChuyenDoiCDS || 0;
  });
  (oppStatus?.rm || []).forEach((r) => {
    const e = ensureRm(r.key, r.phong, r.phongLabel);
    e.oppTotal += r.oppTotal;
    e.oppThanhCong += r.oppThanhCong;
    e.oppTotalCDS += r.oppTotalCDS || 0;
    e.oppThanhCongCDS += r.oppThanhCongCDS || 0;
  });
  (leadInt?.rm || []).forEach((r) => {
    const e = ensureRm(r.key, r.phong, r.phongLabel);
    e.leadTuongTac += r.leadTuongTac;
    e.leadTuongTacCDS += r.leadTuongTacCDS || 0;
  });
  (oppInt?.rm || []).forEach((r) => {
    const e = ensureRm(r.key, r.phong, r.phongLabel);
    e.oppTuongTac += r.oppTuongTac;
    e.oppTuongTacCDS += r.oppTuongTacCDS || 0;
  });
  ...
  validRmRows.forEach((r) => {
    ...
    p.leadGiaoCDS += r.leadGiaoCDS || 0;
    p.leadChuyenDoiCDS += r.leadChuyenDoiCDS || 0;
    p.oppTotalCDS += r.oppTotalCDS || 0;
    p.oppThanhCongCDS += r.oppThanhCongCDS || 0;
    p.leadTuongTacCDS += r.leadTuongTacCDS || 0;
    p.oppTuongTacCDS += r.oppTuongTacCDS || 0;
  });
  if (roster?.labelMap) {
  ...
  const summary = validRmRows.reduce(
    (acc, r) => {
      ...
      acc.leadGiaoCDS += r.leadGiaoCDS || 0;
      acc.leadChuyenDoiCDS += r.leadChuyenDoiCDS || 0;
      acc.oppTongCDS += r.oppTotalCDS || 0;
      acc.oppThanhCongCDS += r.oppThanhCongCDS || 0;
      acc.leadTuongTacCDS += r.leadTuongTacCDS || 0;
      acc.oppTuongTacCDS += r.oppTuongTacCDS || 0;
      return acc;
    },
    {
      leadGiao: 0, leadChuyenDoi: 0, oppTong: 0, oppThanhCong: 0, leadTuongTac: 0, oppTuongTac: 0,
      leadGiaoCDS: 0, leadChuyenDoiCDS: 0, oppTongCDS: 0, oppThanhCongCDS: 0, leadTuongTacCDS: 0, oppTuongTacCDS: 0,
    }
  );
...
export function combineData(list) {
  ...
  const rmCombined = combineRaw('rm').map((e) => ({
    ...
    diemCDS: +(0.3 * (e.leadTuongTacCDS + e.oppTuongTacCDS) + 0.3 * e.leadChuyenDoiCDS + 0.4 * e.oppThanhCongCDS).toFixed(1),
  }));
  ...
  const phongCombined = combineRaw('phong').map((e) => {
    ...
    if (!soRM) return { ...e, soRM: 0, diem: null, diemCDS: null };
    const diem = 0.3 * ((e.leadTuongTac + e.oppTuongTac) / soRM) + 0.3 * (e.leadChuyenDoi / soRM) + 0.4 * (e.oppThanhCong / soRM);
    const diemCDS = 0.3 * ((e.leadTuongTacCDS + e.oppTuongTacCDS) / soRM) + 0.3 * (e.leadChuyenDoiCDS / soRM) + 0.4 * (e.oppThanhCongCDS / soRM);
    return { ...e, soRM, diem: +diem.toFixed(2), diemCDS: +diemCDS.toFixed(2) };
  });
  ...
  const summary = list.reduce(
    (acc, d) => {
      ...
      acc.leadGiaoCDS += s.leadGiaoCDS || 0;
      acc.leadTuongTacCDS += s.leadTuongTacCDS || 0;
      acc.leadChuyenDoiCDS += s.leadChuyenDoiCDS || 0;
      acc.oppTongCDS += s.oppTongCDS || 0;
      acc.oppTuongTacCDS += s.oppTuongTacCDS || 0;
      acc.oppThanhCongCDS += s.oppThanhCongCDS || 0;
      if (s.tongRM) tongRMList.push(s.tongRM);
      return acc;
    },
    {
      leadGiao: 0, leadTuongTac: 0, leadChuyenDoi: 0, oppTong: 0, oppTuongTac: 0, oppThanhCong: 0,
      leadGiaoCDS: 0, leadTuongTacCDS: 0, leadChuyenDoiCDS: 0, oppTongCDS: 0, oppTuongTacCDS: 0, oppThanhCongCDS: 0,
    }
  );
